...
## Load packages 
packs <- c('dplR','plyr','tmap','rasterVis','hexbin','raster','rgdal', 'rvest','reshape2','zoo','nlme',
           'stringr','ggplot2','ecochange', 'dplyr' , 'ggrepel', 'ggpmisc', 'tidyverse','gstat','xtable', 'viridis','R.utils')
...
#Load datasets
dataC  <- read.csv2('F14C_new.csv', dec=".", sep = ";")
dataIg <- read.csv('CO2_Medellin.csv')
co2 <- read.table("co2_annmean_mlo.txt")
names(co2) <- c('Year.crossdating', 'co2', 'V3')
cod  <- read.csv('Code_new.csv')
...
NH3
zone3mean <-NH3%>%group_by(Year.AD)%>%summarize(mean.Delta14C = mean(mean.Delta14C),sd.Delta14C = sd(sd.Delta14C) )%>%filter(Year.AD > 1969)
...
names(dt)
names(dt) <- c(names(zone3mean)[1L], names(dt)[-1L])
merg <- zone3mean%>%left_join(dt)
...
names(zone3mean) <- c('year','14C','sd.14C')
treeCCNH3 <- merge(treeCC, zone3mean,
