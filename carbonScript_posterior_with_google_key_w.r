...
## require(dplyr)
## require(plyr)
## N<- ddply(dt, .(Año.cofechado), nrow)
## Rad_uam<-dt%>%group_by(Año.cofechado)%>%summarize(mean.Delta14C = mean(delta14C),sd.Delta14C = sd(delta14C) )%>%filter(Año.cofechado > 1969)
## Rad_uam<- merge(Rad_uam,N)
####
...
plots <- ggplot(merg, aes(x=Year.AD, y=delta14C)) + 
    geom_point() +
...
ggsave('curve.pdf',plots)


newd <- merge(dt, zona3mean, by.x = "Año.cofechado", by.y = "Year.AD")

newd <- dt%>%left_join(zona3mean)
head(newd)

ggplot(dt, aes(x=Año.cofechado, y=delta14C)) + 
    geom_point()+ylab(expression(Delta^14~C))+ xlab("Time")+
  theme(axis.text =  element_text(face="bold", colour="black", size=rel(0.8)), panel.grid.major = element_blank(), panel.grid.minor =element_blank()) +
  scale_color_manual(values=c('#CD5C5C','#999999')) +scale_size_manual(values=c(24,21))
...
dev.copy(pdf, 'Fig1.pdf')
dev.off()
getwd()


# #ggplot(df1, aes(x=Year, y=X14C.mean , group=Site, color=Site)) + 
#   geom_line() +
#   geom_point()+
#   geom_errorbar(aes(ymin=Icinf, ymax=Icsup), width=1.5,
#                 position=position_dodge(0.05))  +ylab(expression(Delta^14~C))+ xlab("Time")+ 
#   theme(axis.text =  element_text(face="bold", colour="black", size=rel(0.8)), panel.grid.major = element_blank(), panel.grid.minor =element_blank()) +
#   scale_color_manual(values=c('#CD5C5C','#999999')) +scale_size_manual(values=c(24,21))+theme_set(theme_bw())
